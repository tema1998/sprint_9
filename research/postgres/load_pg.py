...
def create_tables():
    with psycopg.connect(
        **settings.get_pg_dsn(), row_factory=dict_row, cursor_factory=ClientCursor
        ...
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS likes (id serial PRIMARY KEY, user_id UUID, film_id UUID, type smallserial, datetime timestamp);"
        )
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS reviews (id serial PRIMARY KEY, user_id UUID, film_id UUID, text text, rating smallserial, datetime timestamp);"
        )
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS bookmarks (id serial PRIMARY KEY, user_id UUID, film_id UUID, datetime timestamp);"
        )
    ...
    with psycopg.connect(
        **settings.get_pg_dsn(), row_factory=dict_row, cursor_factory=ClientCursor
        ...
        for _ in range(number_of_entries):
            ...
            data_values = ", ".join(
                map(lambda x: f"'{str(x)}'", list(fake_data.values()))
            )
            query = (
                f"INSERT INTO {collection_name} ({data_columns}) VALUES ({data_values})"
            )
