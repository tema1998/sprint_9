...
def count_likes_of_film() -> None:
    ...
    with psycopg.connect(**settings.get_pg_dsn(), row_factory=dict_row,
        ...
        cursor.execute(f"SELECT COUNT(*) FROM likes WHERE film_id='{const_film_id}' AND type=10;")
        cursor.fetchall()
    end_time = time.time()
...
def count_average_film_rating() -> None:
    ...
    with psycopg.connect(**settings.get_pg_dsn(), row_factory=dict_row,
                         cursor_factory=ClientCursor) as conn, conn.cursor() as cursor:
        cursor.execute(f"SELECT AVG(rating) FROM reviews WHERE film_id='{const_film_id}' GROUP BY film_id;")
        cursor.fetchall()
        end_time = time.time()
...
def count_number_of_bookmarks() -> None:
    ...
    with psycopg.connect(**settings.get_pg_dsn(), row_factory=dict_row,
                         cursor_factory=ClientCursor) as conn, conn.cursor() as cursor:
        cursor.execute(f"SELECT COUNT(*) FROM bookmarks WHERE user_id='{const_user_id}';")
        cursor.fetchall()
    end_time = time.time()
